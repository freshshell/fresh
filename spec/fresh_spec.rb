...
describe 'fresh' do
  ...
  describe 'using --filter' do
    ...
    it 'ignores output generated by freshrc' do
      file_add fresh_local_path + 'aliases', 'foo other_username bar'
      rc 'echo test stdout before'
      rc 'echo test stderr before >&2'
      rc "fresh aliases --filter='sed s/other_username/my_username/ | tr _ -'"
      rc 'echo test stdout after'
      rc 'echo test stderr after >&2'

      run_fresh success: <<-OUT.strip_heredoc, error: <<-ERR.strip_heredoc, exit_status: true
        test stdout before
        test stdout after
        #{FRESH_SUCCESS_LINE}
      OUT
